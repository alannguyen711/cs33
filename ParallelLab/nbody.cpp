...
int main(int argc, char** argv)
{
  ...
  double power = pow(timestepSeconds, 2.0);
  ...
  for (size_t ts = 0; ts < nTimesteps; ts++) {
    ...
    for (size_t body = 0; body < nBodies; body++) {
      ...
      for (size_t otherBody = 0; otherBody < nBodies; otherBody++) {
        ...
        tempxi += f_i * cos(theta);
        tempyi += f_i * sin(theta);
      }
        bodies[body].f_x += tempxi;
        bodies[body].f_y += tempyi;
    }

    // Movement stage
    #pragma omp parallel for reduction (+:sumAccelerationX, sumAccelerationY, sumDisplacementX, sumDisplacementY) 
    for (size_t body = 0; body < nBodies; body++) {
      ...
      double d_x = 1.0 / 2.0 * a_x * power;
      double d_y = 1.0 / 2.0 * a_y * power;
